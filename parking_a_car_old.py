...
class ParkingCar:
    def __init__(self, q_1_0, q_2_0, q_dot_1_0, q_dot_2_0, phi_0, lambda_0, length = 1.0, nbSteps = 5000):
        ...
        self._q_t[1] = self._q_t[0] + self._dt * self._q_dot_t[0]

        self._phi_t = np.zeros((nbSteps))
        self._phi_dot_t = np.zeros((nbSteps-1))
        self._phi_t[0] = phi_0
        # find phi_dot[0] from 0 = M(q, q_dot, phi, phi_dot)
        self._phi_dot_t[0] = - self._q_dot_t[0][0] * math.cos(self._phi_t[0]) + self._q_dot_t[0][1] * math.sin(self._phi_t[0])
        self._phi_t[1] = self._phi_t[0] + self._dt * self._phi_dot_t[0]

        self._lambda_t = np.zeros((nbSteps-1))
        self._lambda_t[0] = lambda_0

        self._p_t = np.zeros((nbSteps,2))
        # determine p_t[0] 
        self._p_t[0] = self._q_t[0] + np.array([math.sin(self._phi_t[0]), math.cos(self._phi_t[0])])
    ...
    def _Lagrange(self, i, q_t_i_1, q_t_i_2, lambda_t_i_minus_one):
        ...
        self._q_dot_t[i-1][0] = (q_t_i_1 - self._q_t[i-1][0]) / self._dt
        self._q_dot_t[i-1][1] = (q_t_i_2 - self._q_t[i-1][1]) / self._dt
    ...
    def park(self):
        for i in range(2, self._nbSteps):
            # initialize start values

            x_0 = [self._q_t[i-1][0], self._q_t[i-1][1], self._lambda_t[i-2]]

            def _Lagrange_(x):
                q_t_i_1 = x[0]
                q_t_i_2 = x[1]
                lambda_t_i_minus_one = x[2]
                return self._Lagrange(i, q_t_i_1, q_t_i_2, lambda_t_i_minus_one)

            op.root(_Lagrange_, x_0, method='lm', jac=None, tol=None, callback=None, options=None)
